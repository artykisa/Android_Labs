...
class ExeAdapter(val application: Application, var listner:IExercixeAdapterListner, val str_activities:List<String>) :
    RecyclerView.Adapter<ExeAdapter.ViewHolder>() {
    val exersiceRepository: DbRep
    val exersiceList:MutableList<Exercise>
    init {
        exersiceRepository= DbRep(application)
        exersiceList= exersiceRepository.getExersiceList()
    }
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.exe_item, parent, false)
