...
class TimerActivity : AppCompatActivity() {
    ...
    private lateinit var  timeService:forTimer.TimerServiceBinder
    ...
    private val connection=object: ServiceConnection {
        ...
        override fun onServiceConnected(p0: ComponentName, service: IBinder) {
            ...
            if(service is forTimer.TimerServiceBinder)
            {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        timerViewModel= ViewModelProviders.of(this,timerFactory(application,intent.getIntExtra("exeId",1))).get(TimerActivityViewModel::class.java)
        val ft=supportFragmentManager.beginTransaction()
        val fragment=activityListFragment.newInstance(timerViewModel)
        ft.add(R.id.info_activity_list,fragment,INFO_TAG)
        ft.commit()
        tickListner=fragment
        bindService(Intent(this,forTimer::class.java),connection, Context.BIND_AUTO_CREATE)
    ...
    fun onResetClicked(v:View){
        ...
        if(fragment!=null)
            ft.remove(fragment)
        fragment=activityListFragment.newInstance(timerViewModel)
        ft.add(R.id.info_activity_list,fragment,INFO_TAG)
        ft.commit()
    }
    ...
    inner class LayOutChanger{
        fun onNext(){
            if(timerViewModel.canNext())
            {
                val intent=Intent(this@TimerActivity,forTimer::class.java)
                tickListner.onFinish(timerViewModel.currnetActivity)
                var mPlayer = MediaPlayer.create(applicationContext,R.raw.sound);
                mPlayer.start();
                timerViewModel.onNext()
                tickListner.onBegin(timerViewModel.currnetActivity)
                intent.putExtra("action",1)
